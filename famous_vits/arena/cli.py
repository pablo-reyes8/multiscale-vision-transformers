...
from ..data import available_dataset_names, get_dataset_info
from .engine import (
    ARENA_DEFAULTS,
    AUGMENT_PRESETS,
...
def build_parser():
    ...
    parser.add_argument(
        ...
        default=ARENA_DEFAULTS["dataset"],
        help="Built-in dataset shared by every requested model.",
    )
    parser.add_argument(
        "--augment",
    ...
    parser.add_argument(
        "--data-dir", type=str, default="./data", help="Directory used to download/store CIFAR-100."
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="Where the arena stores checkpoints and summaries.",
    )
    parser.add_argument(
        "--img-size", type=int, default=ARENA_DEFAULTS["img_size"], help="Input image size."
    )
    parser.add_argument(
        "--num-classes", type=int, default=None, help="Override inferred dataset classes."
    )
    parser.add_argument(
        "--in-chans",
        type=int,
        default=3,
        help="Input channels (dry-run supports arbitrary values).",
    )
    parser.add_argument(
        "--batch-size", type=int, default=ARENA_DEFAULTS["batch_size"], help="Training batch size."
    )
    parser.add_argument(
        "--eval-batch-size",
        type=int,
    ...
    parser.add_argument(
        "--epochs", type=int, default=ARENA_DEFAULTS["epochs"], help="Epochs per model."
    )
    parser.add_argument(
        "--lr", type=float, default=ARENA_DEFAULTS["lr"], help="Shared learning rate."
    )
    parser.add_argument(
        "--weight-decay",
        type=float,
    ...
    parser.add_argument(
        "--val-split", type=float, default=ARENA_DEFAULTS["val_split"], help="Validation split."
    )
    parser.add_argument("--num-workers", type=int, default=2, help="DataLoader workers.")
    parser.add_argument(
        "--device", type=str, default=None, help="Device identifier, e.g. cpu or cuda."
    )
    parser.add_argument(
        "--autocast-dtype", type=str, default="fp16", help="AMP dtype (fp16 or bf16)."
    )
    ...
    parser.add_argument(
        "--min-lr", type=float, default=ARENA_DEFAULTS["min_lr"], help="Minimum cosine LR."
    )
    parser.add_argument(
        "--label-smoothing",
        type=float,
    ...
    parser.add_argument(
        "--list-models", action="store_true", help="Print available model presets and exit."
    )
...
def main(argv: Sequence[str] | None = None):
    ...
    if args.dry_run:
        num_classes = args.num_classes or get_dataset_info(args.dataset)["num_classes"]
