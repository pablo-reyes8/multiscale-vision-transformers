...
def canonicalize_dataset_name(dataset_name: str) -> str:
    ...
    if key not in DATASET_ALIASES:
        raise ValueError(
            f"Unsupported dataset '{dataset_name}'. Available: {sorted(DATASET_ALIASES.keys())}"
        )
...
def _attach_dataset_metadata(
    dataset, dataset_name: str, num_classes: int, mean, std, class_names=None
):
...
def _copy_dataset_metadata(source, target):
    ...
    if hasattr(source, "classes"):
        target.classes = list(source.classes)
    return target
...
def build_transforms(
    ...
    if augment not in {"matched", "basic", "none", "raw"}:
        raise ValueError("augment must be one of: matched, basic, none, raw.")
    mean = (0.5, 0.5, 0.5) if augment == "raw" else info["mean"]
    std = (0.5, 0.5, 0.5) if augment == "raw" else info["std"]
...
def _build_torchvision_dataset(
    dataset_name: str, split: str, data_dir: str, transform, download: bool
):
...
def _build_tiny_imagenet_datasets(
    ...
    train_dataset, val_dataset = _split_train_val(
        train_aug, train_eval, val_split=val_split, seed=seed
    )
...
def get_classification_datasets(
    ...
    if ddp_safe_download and _ddp_is_on():
        if _ddp_rank() == 0:
            _build_torchvision_dataset(
                dataset_name, "train", data_dir, transform=None, download=True
            )
            _build_torchvision_dataset(
                dataset_name, "test", data_dir, transform=None, download=True
            )
        _ddp_barrier()
    ...
    _attach_dataset_metadata(
        train_aug, dataset_name, info["num_classes"], info["mean"], info["std"], class_names
    )
    _attach_dataset_metadata(
        train_eval, dataset_name, info["num_classes"], info["mean"], info["std"], class_names
    )
    _attach_dataset_metadata(
        test_dataset, dataset_name, info["num_classes"], info["mean"], info["std"], class_names
    )
